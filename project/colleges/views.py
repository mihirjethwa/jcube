...
def index(request):
    ...
    paginator = Paginator(colleges, 12)
...
def listing(request):
    ...
    if request.method == 'POST':
        ...
        #college_list = Map.objects.filter(Q(marks__lte= mark1) & Q(caste__caste= caste) & Q(location_map = location))
        #college_list = Map.objects.filter(Q(marks__lte= mark1) & Q(caste__caste= caste) & Q(f__id= field))
        college_list = Map.objects.filter(Q(f__field_name__iexact= field) & Q(caste__caste__iexact= caste) & Q(marks__lte= mark1) & Q(location_map__exact= location))
        #college_list = map.objects.raw('SELECT * FROM colleges_map WHERE marks < %s AND c_id = %s', [mark,colname])
    ...
    #return render(request, 'colleges/about.html', context=college_dict)
